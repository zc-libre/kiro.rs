...
//!
//! 工具相关的子集（schema 规范化、tool_use/tool_result 配对、超长名称缩短）已下放到
//! [`super::tools`]，本模块通过 use 调用它们。
...
use crate::domain::request::tool::{ToolResult, ToolUseEntry};
...
use super::tools::{
    ...
    extract_tool_result_content, map_tool_name, remove_orphaned_tool_uses, validate_tool_pairing,
};
...
#[cfg(test)]
mod tests {
    use super::super::tools::TOOL_NAME_MAX_LEN;
    use super::*;
    ...
    // shorten_tool_name / map_tool_name / validate_tool_pairing / remove_orphaned_tool_uses
    // / extract_tool_result_content / normalize_json_schema）已迁至
    // src/service/conversation/tools.rs

    #[test]
    fn test_tool_name_mapping_in_convert_request() {
        use crate::interface::http::anthropic::dto::{
    ...
    // validate_tool_pairing 的 6 个函数级单测已迁至 src/service/conversation/tools.rs
    ...
    // remove_orphaned_tool_uses 的 2 个函数级单测已迁至 src/service/conversation/tools.rs
